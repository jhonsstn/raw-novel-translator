...
function jobResponse(job: { id: string }): Response {
...
const importSchema = z
  .object({
    ...
    chapterNumber: z
      ...
      .min(1, 'Chapter number must be positive')
      .max(Number.MAX_SAFE_INTEGER, 'Chapter number must be a safe integer'),
  })
  .strict();
const automationSchema = z
  .object({ autoTranslate: z.boolean().optional(), autoCheck: z.boolean().optional() })
  .strict();
const progressSchema = z
  .object({
    chapterId: z.string().min(1),
    mode: z.enum(['source', 'en']),
    scrollRatio: z.number().finite(),
...
async function metadataBody(request: Request) {
  ...
  if (useSourceTitle !== 'true' && useSourceTitle !== 'false')
    throw new AppError('INVALID_BODY', 'useSourceTitle must be true or false');
  if (
    typeof customTitle !== 'string' ||
    typeof author !== 'string' ||
    typeof description !== 'string' ||
    !['keep', 'remove', 'replace'].includes(String(coverAction))
  )
    throw new AppError('INVALID_BODY', 'Metadata fields are invalid');
  if (useSourceTitle === 'true' && customTitle.trim())
    throw new AppError('INVALID_BODY', 'A custom title cannot be combined with useSourceTitle');
  if (coverAction === 'replace') {
    ...
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type))
      throw new AppError('INVALID_COVER', 'Cover must be JPEG, PNG, or WebP');
  ...
  return {
    customTitle: useSourceTitle === 'true' ? null : customTitle.trim() || null,
...
async function dispatch(request: Request, context: RouteContext): Promise<Response> {
  ...
  if (method === 'GET' && path.length === 1 && path[0] === 'novels')
    return json(listNovels(url.searchParams.get('search') ?? ''));
  ...
  if (method === 'POST' && path[0] === 'novels' && path.length === 3 && path[2] === 'check')
    return jobResponse(queueCheckUpdates(path[1]!));
  ...
  if (method === 'GET' && path.length === 1 && path[0] === 'jobs') {
    ...
    return json(
      listJobs(Number(url.searchParams.get('page') ?? 1), filter === 'active' || filter === 'failed' ? filter : 'all'),
    );
  }
  ...
  if (method === 'POST' && path[0] === 'jobs' && path.length === 3 && path[2] === 'retry')
    return jobResponse(retryJob(path[1]!));
  if (method === 'POST' && path[0] === 'jobs' && path.length === 3 && path[2] === 'cancel')
    return json({ cancelled: cancelJob(path[1]!) });
  if (method === 'GET' && path.length === 1 && path[0] === 'activity')
    return json(listActivity(Number(url.searchParams.get('limit') ?? 100)));
...
async function handle(request: Request, context: RouteContext): Promise<Response> {
  try {
    ...
    if (error instanceof ZodError)
      return json(
        { error: { code: 'INVALID_BODY', message: error.issues[0]?.message ?? 'Request body is invalid' } },
        400,
      );
