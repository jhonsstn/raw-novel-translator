...
export function TtsSettingsPanel() {
  ...
  return (
    <div className="grid gap-4">
      {message && (
        <div className="rounded-[10px] border border-[color-mix(in_srgb,var(--color-success)_30%,var(--color-line))] bg-[color-mix(in_srgb,var(--color-success)_8%,var(--color-card))] px-3.5 py-3 text-success" role="status">
          {message}
        </div>
      )}
      {error && (
        <div className="rounded-[10px] border border-[color-mix(in_srgb,var(--color-danger)_35%,var(--color-line))] bg-danger-soft px-3.5 py-3 text-danger" role="alert">
          {error}
        </div>
      )}
      <form className={`${panelClass} grid gap-5`} onSubmit={save}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="max-w-[680px] text-sm text-muted">
              OpenAI-compatible speech endpoint used by the novel reader. The API key is encrypted at rest and never sent to the browser.
            </p>
          </div>
          <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${configured ? 'bg-[color-mix(in_srgb,var(--color-success)_12%,var(--color-card))] text-success' : 'bg-paper-raised text-muted'}`}>
        ...
        <div className="grid grid-cols-2 gap-3.5 max-[760px]:grid-cols-1">
          <label className={fieldClass}>
            ...
            <input className={inputClass} type="url" required placeholder="https://api.openai.com/v1" value={settings.baseUrl ?? ''} onChange={(event) => setSettings({ ...settings, baseUrl: event.target.value })} />
          </label>
          <label className={fieldClass}>
            Model
            <input className={inputClass} required placeholder="gpt-4o-mini-tts" value={settings.model ?? ''} onChange={(event) => setSettings({ ...settings, model: event.target.value })} />
          </label>
          <label className={fieldClass}>
            API key
            <input className={inputClass} type="password" placeholder={settings.hasApiKey ? 'Stored — enter to replace' : 'Required'} value={apiKey} onChange={(event) => setApiKey(event.target.value)} />
          </label>
          <label className={fieldClass}>
            Voice
            <input className={inputClass} list="tts-voice-options" required value={settings.voice} onChange={(event) => setSettings({ ...settings, voice: event.target.value })} />
            <datalist id="tts-voice-options">{voices.map((voice) => <option value={voice} key={voice} />)}</datalist>
          </label>
          <label className={fieldClass}>
            Timeout (seconds)
            <input className={inputClass} type="number" min="10" max="300" value={settings.timeoutSeconds} onChange={(event) => setSettings({ ...settings, timeoutSeconds: Number(event.target.value) })} />
          </label>
        </div>
        <div className="grid grid-cols-2 gap-5 max-[760px]:grid-cols-1">
          <label className={fieldClass}>
            <span className="flex items-center justify-between gap-3"><span>Speed</span><strong>{settings.speed.toFixed(2)}×</strong></span>
            <input className="accent-[var(--color-accent)]" type="range" min="0.25" max="4" step="0.05" value={settings.speed} onChange={(event) => setSettings({ ...settings, speed: Number(event.target.value) })} />
            <span className="font-normal text-muted">Sent as the standard OpenAI-compatible speed parameter.</span>
          </label>
          <label className={fieldClass}>
            <span className="flex items-center justify-between gap-3"><span>Pitch</span><strong>{settings.pitch > 0 ? '+' : ''}{settings.pitch} st</strong></span>
            <input className="accent-[var(--color-accent)]" type="range" min="-12" max="12" step="1" value={settings.pitch} onChange={(event) => setSettings({ ...settings, pitch: Number(event.target.value) })} />
            <span className="font-normal text-muted">Pitch is expressed through speech instructions. Legacy TTS models may ignore it.</span>
          </label>
        </div>
        <div className="flex flex-wrap items-center gap-[9px]">
          ...
          <button className={buttonClass} type="button" disabled={testing || !configured} onClick={() => void testVoice()}>
            {testing ? <Loader2 className="animate-spin" size={16} /> : <Play size={16} />}
            {testing ? 'Generating…' : 'Test saved voice'}
          </button>
        </div>
        <p className="text-xs text-muted">The voice preview uses the last saved provider settings. Save changes before testing them.</p>
      </form>
      <section className={panelClass}>
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="mb-1 flex items-center gap-2"><CheckCircle2 size={17} className="text-accent-ink" /><strong>Continuous reading</strong></div>
            <p className="max-w-[640px] text-sm text-muted">When narration reaches the end of a chapter, continue into the next available chapter automatically.</p>
          </div>
          <button
