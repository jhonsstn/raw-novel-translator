  ...
  return (
    <>
      ...
      <div className="toolbar" style={{ marginBottom: 22 }}>
        <label style={{ position: 'relative', maxWidth: 380, width: '100%' }}>
      ...
      {novels.length === 0 ? (
        ...
        <div className="grid">
          {novels.map((novel) => (
            <Link className="card novel-card" href={`/novels/${novel.id}`} key={novel.id}>
              <div className="cover">
                {novel.coverUrl && <img src={novel.coverUrl} alt="" />}
                <div className="cover-placeholder">{novel.displayTitle}</div>
              </div>
              <div className="card-body">
                <h3>{novel.displayTitle}</h3>
                <div className="muted">{novel.author ?? 'Unknown author'}</div>
                <div className="meta">
                  <span className="badge">
                    {novel.downloadedCount}/{novel.chapterCount} downloaded
                  </span>
                  <span className="badge">{novel.translatedCount} translated</span>
                  {novel.currentChapterId && <span className="badge">Continue reading</span>}
                </div>
              </div>
            </Link>
          ))}
        ...
        <div className="dialog-backdrop" role="presentation">
          <div className="dialog" role="dialog" aria-modal="true" aria-labelledby="import-title">
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              ...
              <button className="btn" aria-label="Close" disabled={pending} onClick={() => setOpen(false)}>
                <X size={17} />
              </button>
            </div>
            <form className="stack" onSubmit={submit} aria-busy={pending}>
              <p className="muted" id="import-metadata-help">
              ...
              <p className="muted" id="import-number-help">
