  ...
  for (const sentence of sentences) {
    ...
    if (chars.length > maxCharacters) {
      ...
      for (let offset = 0; offset < chars.length; offset += maxCharacters)
        chunks.push(chars.slice(offset, offset + maxCharacters).join('').trim());
      continue;
    }
...
export function ReaderTtsControls({ chapterId, language, paragraphs, contentRef, hasNextChapter, onAutoNext }: Props) {
  ...
  const fetchAudio = useCallback(async (index: number, part: number, signal?: AbortSignal): Promise<string> => {
  ...
  const playSegment = useCallback(async (index: number, part: number) => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      generationRef.current += 1;
      void playSegmentRef.current(0, 0);
    }, 250);
    return () => clearTimeout(timer);
  }, [configured, chapterId, paragraphs]);
  ...
  async function togglePlayback() {
    ...
    generationRef.current += 1;
    await playSegment(firstVisibleParagraph(), 0);
  }

  const label = state === 'loading' ? 'Generating…' : state === 'playing' ? 'Pause' : state === 'paused' ? 'Resume' : 'Read aloud';
  const progress = paragraphIndex === null || !paragraphs?.length ? null : `${paragraphIndex + 1} / ${paragraphs.length}`;
  return (
    <div className="mx-auto mb-4 w-[min(780px,100%)] rounded-[14px] border border-line bg-card px-3.5 py-3 shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <div className="grid size-9 shrink-0 place-items-center rounded-[10px] bg-[color-mix(in_srgb,var(--color-accent)_11%,var(--color-card))] text-accent-ink"><Volume2 size={17} /></div>
          <div className="min-w-0">
            <div className="text-sm font-bold">Narration</div>
            <div className="truncate text-xs text-muted">{progress ? `Paragraph ${progress} · ${language === 'en' ? 'English' : '中文'}` : 'Starts from the first visible paragraph'}</div>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button className={state === 'idle' || state === 'paused' ? primaryButtonClass : buttonClass} disabled={!configured || !paragraphs?.length || state === 'loading'} onClick={() => void togglePlayback()}>
            {state === 'loading' ? <Loader2 className="animate-spin" size={16} /> : state === 'playing' ? <Pause size={16} /> : <Play size={16} />}
            {label}
          </button>
          <button className={buttonClass} disabled={state === 'idle'} onClick={stop} aria-label="Stop narration"><Square size={15} /></button>
        </div>
      </div>
      {configured === false && <p className="mt-2 text-xs text-warning">Configure a text-to-speech provider in Settings before using narration.</p>}
