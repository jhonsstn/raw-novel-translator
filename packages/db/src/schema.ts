...
import { blob, check, index, integer, real, sqliteTable, text, uniqueIndex, type AnySQLiteColumn } from 'drizzle-orm/sqlite-core';
...
export const novelCovers = sqliteTable('novel_covers', {
  novelId: text('novel_id').primaryKey().references(() => novels.id, { onDelete: 'cascade' }),
...
export const translationChunks = sqliteTable('translation_chunks', {
  runId: text('run_id').notNull().references(() => translationRuns.id, { onDelete: 'cascade' }),
