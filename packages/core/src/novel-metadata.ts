...
function normalizeText(input: NovelMetadataInput): { customTitle: string | null; description: string | null } {
  ...
  if (description && codePointLength(description) > 10_000) throw new AppError('INVALID_DESCRIPTION', 'Description is limited to 10,000 characters');
...
export async function normalizeCover(bytes: Uint8Array): Promise<NormalizedCover> {
  ...
  try {
    ...
    if (metadata.format !== signature || !metadata.width || !metadata.height || (metadata.pages ?? 1) !== 1) throw new AppError('INVALID_COVER', 'Cover must be one valid static JPEG, PNG, or WebP image');
    const result = await image.rotate().resize({ width: 800, height: 1200, fit: 'inside', withoutEnlargement: true }).webp({ quality: 85 }).toBuffer({ resolveWithObject: true });
    if (result.data.byteLength > 2 * 1024 * 1024) throw new AppError('COVER_TOO_LARGE', 'Normalized cover exceeds 2 MiB', 413);
    if (!result.info.width || !result.info.height) throw new AppError('INVALID_COVER', 'Cover dimensions are invalid');
    return { bytes: result.data, hash: createHash('sha256').update(result.data).digest('hex'), width: result.info.width, height: result.info.height };
  } catch (error) {
...
export async function updateNovelMetadata(novelId: string, input: NovelMetadataInput): Promise<void> {
  ...
  sqlite.transaction(() => {
    ...
    if (cover) sqlite.prepare(`INSERT INTO novel_covers(novel_id,image,content_hash,width,height,updated_at) VALUES (?,?,?,?,?,?)
      ON CONFLICT(novel_id) DO UPDATE SET image=excluded.image,content_hash=excluded.content_hash,width=excluded.width,height=excluded.height,updated_at=excluded.updated_at`).run(novelId, cover.bytes, cover.hash, cover.width, cover.height, now);
  })();
}
