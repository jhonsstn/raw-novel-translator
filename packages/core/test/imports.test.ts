...
vi.mock('@novel/sources', async (importOriginal) => {
  ...
  return { ...actual, createSourceTransport: () => async (url: string) => {
...
afterEach(() => { database.closeDatabase(); rmSync(directory, { recursive: true }); });

async function runImport(includeStart = true) {
  const queued = imports.startImport({ url: 'https://www.piaotia.com/html/10/20/17.html', includeStart, title: ' My title ', description: ' My description\r\nSecond line ', chapterNumber: 42 });
...
it('uses manual metadata and numbering, preserving them when directory positions shift', async () => {
  ...
  expect(novel).toMatchObject({ displayTitle: 'My title', description: 'My description\nSecond line', author: null, coverUrl: null });
...
it('starts at the next entered number when the supplied chapter is excluded', async () => {
  ...
  expect(library.getNovel(id).chapters.map(chapter => [chapter.ordinal, chapter.canonicalUrl])).toEqual([[43, 'https://www.piaotia.com/html/10/20/600.html']]);
});

it('rejects legacy import jobs instead of scraping missing manual metadata', async () => {
  const legacy: Job = { ...jobs.enqueueJob({ kind: 'import', payload: { url: 'https://www.piaotia.com/html/10/20/17.html', includeStart: true } }), leaseToken: 'lease' };
  await expect(imports.handleImportJob(legacy, new AbortController().signal)).rejects.toMatchObject({ code: 'INVALID_JOB' });
