...
describe('persistent job diagnostics', () => {
  it('retains failed history and links retries across database reopen', () => {
    ...
    jobs.failJob(
      original.id,
      claimed.leaseToken!,
      new Error('fetch failed', { cause: Object.assign(new Error('connection refused'), { code: 'ECONNREFUSED' }) }),
    );
  ...
  it('keeps reclaimed attempts and rejects stale or cancelled lease writes', () => {
    ...
    expect(before.filter((event) => event.message === 'Job claimed by worker').map((event) => event.attempt)).toEqual([
      1, 2,
    ]);
  ...
  it('persists nested fetch causes while redacting credentials and ignoring arbitrary fields', () => {
    ...
    const job = jobs.enqueueJob({
      kind: 'source_check',
      payload: { sourceId: 'test-source', privatePayload: 'PRIVATE_PAYLOAD' },
    });
    const claimed = jobs.claimJob('source', Date.now())!;
    jobs.recordJobEvent(
      job.id,
      claimed.leaseToken!,
      'Request context',
      'GET https://username:password@source.example/novel?token=QUERY_SECRET Authorization: Bearer HEADER_SECRET api-key=KEY_SECRET',
    );
    ...
    expect(activity.listJobEvents(job.id).find((event) => event.details?.includes('ECONNREFUSED'))?.details).toContain(
      'at ',
    );
  });
  ...
  it('bounds cyclic diagnostics and returns only the latest 200 events oldest-first', () => {
    ...
    const large = new AggregateError(
      Array.from({ length: 30 }, () => new Error('x'.repeat(20_000))),
      'many errors',
    );
